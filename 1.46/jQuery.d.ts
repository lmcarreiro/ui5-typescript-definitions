...
declare interface JQuerySap {
     ...
     * @param {Function} fnErrorCallback - callback function to get notified once the stylesheet loading failed.            In case of usage in IE the error callback will also be executed if an empty stylesheet            is loaded. This is the only option how to determine in IE if the load was successful            or not since the native onerror callback for link elements doesn't work in IE. The IE            always calls the onload callback of the link element.            Another issue of the IE9 is that in case of loading too many stylesheets the eventing            is not working and therefore the error or load callback will not be triggered anymore.
     * @returns {void|Promise<any>} When using the configuration object a <code>Promise</code> will be returned. The            documentation for the <code>fnLoadCallback</code> applies to the <code>resolve</code>            handler of the <code>Promise</code> and the one for the <code>fnErrorCallback</code>            applies to the <code>reject</code> handler of the <code>Promise</code>.
     */
